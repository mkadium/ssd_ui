...
| Flow ID | Flow | Status |
|---|---|---|
| AUTH-FLOW-001 | User opens `/login`, submits login identifier, password, and optional unit id, then enters authenticated portal on successful `POST /auth/login` | STARTED |
...
| AUTH-FLOW-007 | User selects a demo role profile for UI review and enters Super Admin or Unit Admin dashboard with sample Auth API-shaped data | IMPLEMENTED_SAMPLE_ONLY |
