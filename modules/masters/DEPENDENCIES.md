# DEPENDENCIES

| Dependency | Purpose | Status |
|---|---|---|
| `ssd_api/modules/masters/API_CONTRACTS.md` | Masters API contract | DEV_AUTHENTICATED_ENDPOINT_EVIDENCE_CAPTURED |
| Auth UI/session | Bearer auth and permission-aware shell | PENDING_UI_IMPLEMENTATION |
| React/Vite/TypeScript/Tailwind | UI stack | APPROVED |
| TanStack Query | Server state | APPROVED |

## Risks
- Mutation CRUD is not available yet.
- Unapproved merged dependencies must not be expanded without governance approval.
