...
- Indicator Library can create the base national indicator record using the selected unit's active framework/edition. Global indicator mapping and metadata update flows are editable through right-side drawer forms where integrated.
- Indicator list source ministry/department, UOM, periodicity, and global mapping display should come from `/masters/indicators` list-summary fields. API backs published-template-derived summary fields with the normal SQL read-model view `metadata.v_indicator_published_template_usage`; UI must not perform per-row detail calls for these columns. Indicator Detail operational usage must come from `published_template_usage`; do not fall back to direct indicator source assignments for source/UOM/periodicity/measure/officer display.
...
- Indicator detail Mapping tab no longer shows a separate Framework section. Only Global Indicator Mapping is editable from Indicator detail. Source, periodicity, source officer, UOM, and measure sections are read-only and must be derived from `published_template_usage`. Do not add direct map/unmap controls for those sections unless governance approves a separate indicator-reference-defaults workflow.
...
- Data Field Library is implemented under `/data-fields/library` using WP-2026-011 Masters APIs. It treats indicator measures as schedulable data fields with source and periodicity mappings.
...
- Data Field Library uses `GET /masters/data-fields` for table rows. Table `Used / Status` shows template usage plus mapped/published indicator count where supplied by DB. Row click opens a dedicated detail workspace through `/data-fields/library?measure={measure_code}`, which uses `GET /masters/data-fields/{measure_code}` for the selected profile, existing indicator-version measure create/update routes for Data Field CRUD, and mapping routes for source, periodicity, unmap, and restore operations.
- Data Field source and periodicity mapping drawers use stable-code selections for source organizations and periodicities.
- Data Field Library must not treat grain as a default measure-owned mapping for new UI work. Grain is template-specific because the same measure can be collected at different coordinate structures by template/source/indicator.
- Template Grain Usage in Data Field Library is read-only and derived from saved Template Studio design state. It groups usage by template, indicator, source, and actual template axes such as `2026 - Total / Rural / Urban - States`.
...
- Do not implement Data Field source/periodicity mappings as UI-only state. Use the WP-2026-011 API mapping routes and stable-code persistence.
- Do not create new measure-level grain UI. Collection grain belongs to Template Studio/template usage unless governance approves a separate shared default-grain feature later.
