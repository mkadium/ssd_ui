...
- Put new active UI implementation only under `ssd_ui/frontend/`.
- The active Vite app scaffold is organized under `ssd_ui/frontend/src/` with shared folders for `api`, `assets`, `components`, `hooks`, `layouts`, `lib`, `pages`, `providers`, `routes`, and `stores`.
- Treat `ssd_ui/ui/` as the previous scaffold/reference unless the Governance Owner explicitly redirects work there.
- Keep module ownership traceable through `ssd_ui/modules/<module>/` even when implementation files live in shared `src` folders.
- Do not create `ssd_ui/frontend/modules/<module>/`; `ssd_ui/modules/<module>/` is the context/contract folder only.
